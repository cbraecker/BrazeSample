	...
	void calculateCurrentLog()
	{
		...
		if (collapse) {
			...
			for (int i = 0; i < logs.Count; i++) {
				...
				switch (log.logType)
				{
					case _LogType.Log when !showLog:
					case _LogType.Warning when !showWarning:
					case _LogType.Error when !showError:
					case _LogType.Assert when !showError:
					case _LogType.Exception when !showError:
						continue;
				}
...
//#if UNITY_CHANGE3
//		currentScene = SceneManager.GetActiveScene().name ;
//		Palast.Log( "Scene " + SceneManager.GetActiveScene().name + " is loaded");
//#else
//		currentScene = Application.loadedLevelName;
//		Palast.Log("Scene " + Application.loadedLevelName + " is loaded");
//#endif
//	}
